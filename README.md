...
ssl_csr_local_dev         : `自分で設定`
ssl_csr_remote_staging    : `自分で設定`
ssl_csr_remote_production : `自分で設定`
...
2. プロビジョニングが止まった場合は、プロジェクトルートで`$ vagrant provision`コマンドを打つと再開できる
...
※ 【III-3】を実施した後は、上記4のコマンドが使えないので、以下で実施する
...
※ 本番サーバのSSLには自己証明書を使っているが、一般公開する場合は、正規の証明書を購入すること
...
- 仮想マシンの場合、「`【II-3】で自分で設定したIPアドレス:【II-4】で自分で設定したポート番号`」でアクセスできる
- 本番サーバーの場合、「`本番サーバーのドメイン名:【II-4】で自分で設定したポート番号`」でアクセスできる
